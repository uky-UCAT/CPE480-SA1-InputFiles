...
configInputFilePath = "./SA1SE3/input/parameterconfig.txt"
configs=[]
##################################################################################
##                                                                              ##           
...
configFilePath= scriptDirectoryPath+"SA1SE3/config/"
# Resultant File path where the result of executing the config for different programs needs to be stored 
resultFilePath=scriptDirectoryPath+"SA1SE3/result/"
# Resultant File path where ptrace of the executed commands is stored 

ptraceFilePath=scriptDirectoryPath+"SA1SE3/ptrace/"
# Results of File path where pipeview of the ptrace commands is stored 
pipeviewFilePath = scriptDirectoryPath+"SA1SE3/pipeview/"
#
assemblyFilePath = scriptDirectoryPath+"SA1SE3/assembly/"


defaultConfigurationFile ="sa1se3-default.cfg"
commandPostFix=""
...
configFilePath = "SA1SE3/simulationstats/"
configList = [f for f in listdir(configFilePath) if isfile(join(configFilePath, f))]
compareFilesList = []

for config in configList:
    compareFilesList.append("SA1SE3/simulationstats/"+config)
compareFiles = []
...
resultFileName = "sa1SE3-result"
combined_df.to_csv('SA1SE3/comparison/'+resultFileName+'.csv')
...
for pipeview in pipeviewFileList:
    ...
    X = (int)(lineno/3)
    print("X",X)
    print("Assembly at X =>",lines[pipeviewassemblymap[X]])
    # print("Assembly at X =>",lines[X])
    # print("Assembly at X =>",lines[X+1])
    Xend = X+100
    
    print("Assembly at X end=>",lines[pipeviewassemblymap[Xend]])
    Y = 2*(int)(lineno/3)
    Yend = Y+100
    ...
    while lines[temp].__contains__("@")== False:
        temp = temp-1
        print("Lines of temp >",lines[temp])
    XCycleNo = lines[temp]
    ...
    print(pipeviewassemblymap[X])
    print(temp)
    XlinesListAssembly = lines[pipeviewassemblymap[X]:temp]
    print(len(XlinesListAssembly))
    ...
    xtracefile.close()
    print("XcycleNo --->",XCycleNo)
    print("XencCycleNo --->",XendCycleNo)       
    ...
    print(pipeviewassemblymap[Y])
    print(temp)
    YlinesListAssembly = lines[pipeviewassemblymap[Y]:temp]
    print(len(YlinesListAssembly))
    ...
    Ytracefile = open(ptraceFilePath+"Y_"+pipeview.replace('txt','trc'),'w')
    for line in YlinesListTrace:
        Ytracefile.write(line)
    Ytracefile.close()
    print("YcycleNo --->",YCycleNo)
    print("YencCycleNo --->",YendCycleNo) 
...
# assemblyFileList = [f for f in listdir(assemblyFilePath) if isfile(join(assemblyFilePath, f))]
# print(assemblyFileList)
